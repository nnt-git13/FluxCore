...
//
//   For W = 1 this reduces exactly to the original single-word behavior:
//   one stall cycle, fill data captured at posedge N+1, WB reads it in N+2.
//
//   In the last FILL cycle the module keeps mem_addr_o at the final beat
//   address so the backing BRAM re-registers it. This keeps mem_rdata_i
//   stable in the following cycle — harmless since wb_stage reads
//   cpu_rdata_o (the registered capture), not mem_rdata_i directly.
//
...
//   On a store hit the cached word is also updated. Store misses skip cache fill.
...
//                Default 64.
//   LINE_WORDS — 32-bit words per cache line. Must be a power of two, 1..16
//                (16 = the largest burst mem_if_pkg can express, so lines stay
//                expressible as single transactions when the mem_if backend
//                arrives). Default 1 → identical geometry and cycle behavior
//                to the original single-word design.
...
#(
    ...
    logic [31:0]           data_q   [0:NSETS-1][0:LINE_WORDS-1];
    ...
    always @(*) begin : comb
        ...
        store_merged_s = data_q[index_s][woff_s];
        ...
        if (rst) begin
            ...
            case (state_q)
                ST_IDLE: begin
                    if (cpu_ren_i) begin
                        if (hit_s) begin
                            cpu_rdata_q <= data_q[index_s][woff_s];
                    ...
                    if (cpu_wen_i && hit_s)
                        data_q[index_s][woff_s] <= store_merged_s;
                end
                ST_FILL: begin
                    // Capture the beat that arrived this cycle.
                    data_q[miss_index_q][beat_q] <= mem_rdata_i;
                    if (beat_q == miss_woff_q)
                        cpu_rdata_q <= mem_rdata_i;
                    if (beat_q + 1 == LINE_WORDS) begin
                        // Line complete: publish tag/valid, return to IDLE.
